import { ELEMENTS, atomValenceElectronMap, countValenceElectrons } from '../chemistry/elements.js';
...
export function buildValenceBasis(atoms, basisScaleOrOptions = 1, maybeOptions = {}) {
  const normalizedOptions = typeof basisScaleOrOptions === 'number'
    ? { basisScale: basisScaleOrOptions, ...maybeOptions }
    : basisScaleOrOptions;
  const basisScale = normalizedOptions.basisScale ?? 1;
  const totalCharge = normalizedOptions.totalCharge ?? 0;
  const electronCountOverride = normalizedOptions.electronCount;
  ...
  return {
    functions: basis,
    electronCount: electronCountOverride ?? countValenceElectrons(atoms, totalCharge),
    atomValenceMap: atomValenceElectronMap(atoms)
  };
}
