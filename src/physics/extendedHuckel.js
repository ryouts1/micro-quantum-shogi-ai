...
function inferReactiveIds(atoms) {
  return {
    nucleophileId: atoms.find((atom) => atom.role === 'nucleophile')?.id,
    carbonId: atoms.find((atom) => atom.role === 'carbon')?.id,
    leavingGroupId: atoms.find((atom) => atom.role === 'leaving-group')?.id,
    nucleophileHydrogenIds: atoms.filter((atom) => atom.role === 'nucleophile-hydrogen').map((atom) => atom.id)
  };
}
...
export function solveExtendedHuckel(atoms, options = {}) {
  ...
  const totalCharge = options.totalCharge ?? 0;

  const basisModel = buildValenceBasis(atoms, { basisScale, totalCharge });
  ...
  const reactiveIds = inferReactiveIds(atoms);
  ...
  const nucleophileHydrogenPopulation = reactiveIds.nucleophileHydrogenIds.length
    ? reactiveIds.nucleophileHydrogenIds.reduce((sum, hydrogenId) => (
      sum + computeBondOverlapPopulation(reactiveIds.nucleophileId, hydrogenId, densityMatrix, overlapMatrix, basisByAtom)
    ), 0) / reactiveIds.nucleophileHydrogenIds.length
    : null;
  ...
  const model = {
    ...
    reactiveIds,
    overlapPopulations: {
      nucleophileCarbon: computeBondOverlapPopulation(reactiveIds.nucleophileId, reactiveIds.carbonId, densityMatrix, overlapMatrix, basisByAtom),
      carbonLeavingGroup: computeBondOverlapPopulation(reactiveIds.carbonId, reactiveIds.leavingGroupId, densityMatrix, overlapMatrix, basisByAtom),
      nucleophileHydrogenAverage: nucleophileHydrogenPopulation
    },
    homoIndex,
    lumoIndex,
